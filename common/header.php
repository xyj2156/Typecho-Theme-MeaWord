                ...
                <div class="top_right">
                    <nav class="header-menu">
                        <ul class="header-menu-ul">
                            <li id="menu-item-home" class=" menu-item menu-item-type-custom menu-item-object-custom
                                <?php if ($this->is('index')): ?>current-menu-item<?php endif; ?> current_page_item menu-item-home
                            "><a href="<?php $this->options->siteUrl(); ?>" aria-current="page">首页</a></li>
                            <?php $this->widget('Widget_Metas_Category_List')->to($categorys); ?>
                            <?php while ($categorys->next()): ?>
                                <?php if ($categorys->levels === 0): ?>
                                    <?php $children = $categorys->getAllChildren($categorys->mid); ?>
                                    <li id="menu-item-<?php $categorys->mid(); ?>"
                                        class="menu-item menu-item-type-taxonomy menu-item-object-category <?php if ($this->is('category', $categorys->slug)): ?> current-menu-item<?php endif; ?> menu-item-<?php $categorys->mid(); ?>">
                                        <a href="<?php $categorys->permalink(); ?>"><?php $categorys->name(); ?></a></li>
                                <?php endif; ?>
                            <?php endwhile; ?>
                            <?php $this->widget('Widget_Contents_Page_List')->to($pages); ?>
                            <?php while ($pages->next()): ?>
                                <li class="menu-item <?php if ($this->is('page', $pages->slug)): ?>current-menu-item<?php endif; ?>"><a
                                            href="<?php $pages->permalink(); ?>" title="<?php $pages->title(); ?>"><?php $pages->title(); ?></a></li>
                            <?php endwhile; ?>
                        </ul>
                    </nav>
                    <div class="top_icon" style="display: flex;">
                        ...
                        <div class="top_icon_btn collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseSearch" aria-expanded="false"
                             aria-controls="collapseSearch"><i class="bi bi-search collapse-show"></i><i class="bi bi-x-circle collapse-hide"></i></div>
                        <button class="top_icon_btn d-lg-none" type="button" data-bs-toggle="offcanvas" data-bs-target="#right-canvas"><i
                                    class="bi bi-list"></i></button>
                    </div>
                </div>
            </div>
        </div>
        <div class="collapse collapseSearch"
             id="collapseSearch"<?php echo ($this->is('index') && !empty($bannerIdsArr[0])) ? ' style="position: absolute;"' : ''; ?>>
