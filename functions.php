...
Typecho_Plugin::factory('admin/footer.php')->end               = ['Meayair', 'editOptionsTheme'];
Typecho_Plugin::factory('Widget_Abstract_Contents')->excerptEx = ['Meayair', 'blockquoteExtra'];
Typecho_Plugin::factory('Widget_Abstract_Contents')->contentEx = ['Meayair', 'blockquoteExtra'];
...
class Meayair
{
    ...
    public static function editOptionsTheme()
    {
        ...
        if ($fileNameCut[0] == 'options-theme') {
            ...
            $css = '<style>#custom-field input,textarea{width:100%;}.wmd-button-row{height:auto;}</style>';
            echo $css;
...
function themeConfig($form)
{
    ...
    $logoDisplay = new Typecho_Widget_Helper_Form_Element_Select('logoDisplay', [
        2 => '仅显示LOGO',
        1 => '仅显示标题',
        0 => 'logo,标题同时显示',
    ...
    $needAjaxScroll = new Typecho_Widget_Helper_Form_Element_Select('needAjaxScroll', [
        1 => '开启',
        0 => '不开启',
...
function getNextPrev($mode, $archive, $fields_name = '')
{
    ...
    if ($content) {
        ...
        if (empty($fields_name)) {
            return compact('fields', 'title', 'permalink');
        }
        ...
        return ['fields' => $fields, 'title' => $title, 'permalink' => $permalink];
    }
    return false;
}
